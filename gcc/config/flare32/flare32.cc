...
//#define FLARE32_CONSTRAINT_4ADDR_DEBUG
...
#ifdef FLARE32_CONSTRAINT_4ADDR_DEBUG
#define flare32_constraint_4addr_debug_fprintf(...) \
  do { fprintf (__VA_ARGS__); } while (0)
#define flare32_constraint_4addr_debug_rtx(...) \
  do { debug_rtx (__VA_ARGS__); } while (0)
#else
#define flare32_constraint_4addr_debug_fprintf(...) do { } while (0)
#define flare32_constraint_4addr_debug_rtx(...) do { } while (0)
#endif
...
flare32_asm_trampoline_template (FILE *f)
{
  ...
  // Flare32 that do not have any data cache.
  asm_fprintf (f, "\tadd %r, #-4\n", STACK_POINTER_REGNUM);
  asm_fprintf (f, "\tpush r0\n");

  // 0x2 == 0x0 + 0x2
  asm_fprintf (f, "\tadd r0, pc, #0xe - 0x2\n");

  // 0x4 == 0x2 + 0x2
  asm_fprintf (f, "\tldr %r, [r0]\n", STATIC_CHAIN_REGNUM);

  // 16-bit instruction encoding, due to relaxing in GAS, so we don't need
  // to use `str.nr`.
  // 0x6 = 0x4 + 0x2
  asm_fprintf (f, "\tldr r0, [r0, #0x4]\n");

  // 16-bit instruction encoding, due to relaxing in GAS, so we don't need
  // to use `str.nr`.
  // 0x8 = 0x6 + 0x2
  asm_fprintf (f, "\tstr r0, [%r, #8]\n", STACK_POINTER_REGNUM);

  // 0xa = 0x8 + 0x2
  asm_fprintf (f, "\tpop r0\n");
  ...
  addr_space_t as)
{
  ...
  //if (GET_MODE_SIZE (mode) == UNITS_PER_WORD)
  {
    ...
    )
    {
      ...
      {
        if (flare32_valid_index_reg_p (right_0, strict_p)
          || flare32_absolute_ex (right_0))
        {
        ...
        //else if (GET_CODE (left_0) == PLUS)
        //{
        //  rtx
        //    left_1 = XEXP (left_0, 0),
        //    right_1 = XEXP (left_0, 1);
        //  if (flare32_valid_index_reg_p (left_1, strict_p)
        //    && flare32_absolute_ex (right_1))
        //  {
        //    return true;
        //  }
        //}
      ...
      if (GET_CODE (left_0) == PLUS)
      {
        rtx
          left_1 = XEXP (left_0, 0),
          right_1 = XEXP (left_0, 1);
        if (flare32_valid_base_reg_p (left_1, strict_p)
          && flare32_valid_index_reg_p (right_1, strict_p)
          && flare32_absolute_ex (right_0))
        {
          return true;
        }
    ...
    //{
    //}
    //if (flare32_valid_base_reg_p (left_0, strict_p)
...
{
  bool ret = false;
  flare32_constraint_4addr_debug_fprintf (stderr,
    "(32-bit offset base + index + imm) start:\n");
  flare32_constraint_4addr_debug_rtx (x);
  //flare32_constraint_4addr_debug_fprintf (stderr, "\n");
        ...
        if (GET_CODE (left_1) == SUBREG)
        {
          ...
          left_1
            = simplify_gen_subreg (SImode, left_1, DImode,
              subreg_lowpart_offset (SImode, DImode));
          flare32_constraint_4addr_debug_fprintf (stderr, "left_1 SUBREG:\n");
          flare32_constraint_4addr_debug_rtx (left_1);
        }
        if (GET_CODE (right_1) == SUBREG)
        {
          ...
          right_1
            = simplify_gen_subreg (SImode, right_1, DImode,
              subreg_lowpart_offset (SImode, DImode));
          flare32_constraint_4addr_debug_fprintf (stderr, "right_1 SUBREG:\n");
          flare32_constraint_4addr_debug_rtx (right_1);
        }
        //#endif  // REG_OK_STRICT
