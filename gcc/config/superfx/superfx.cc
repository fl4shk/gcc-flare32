...
#include "superfx-protos.h"

__attribute__ ((used)) void
superfx_dump_file_debug_rtx (FILE* file, const_rtx x)
{
...
#define SUPERFX_DEBUG
//#define SUPERFX_CONSTRAINT_DEBUG
//#define SUPERFX_CONSTRAINT_4ADDR_DEBUG

#ifdef SUPERFX_DEBUG
#define superfx_debug_fprintf(...) \
  do { fprintf (__VA_ARGS__); } while (0)
#define superfx_debug_rtx(...) \
  do { debug_rtx (__VA_ARGS__); } while (0)
#else
#define superfx_debug_fprintf(...) do { } while (0)
#define superfx_debug_rtx(...) do { } while (0)
#endif

#ifdef SUPERFX_CONSTRAINT_DEBUG
#define superfx_constraint_debug_fprintf(...) \
  do { fprintf (__VA_ARGS__); } while (0)
#define superfx_constraint_debug_rtx(...) \
  do { debug_rtx (__VA_ARGS__); } while (0)
#else
#define superfx_constraint_debug_fprintf(...) do { } while (0)
#define superfx_constraint_debug_rtx(...) do { } while (0)
#endif

#ifdef SUPERFX_CONSTRAINT_4ADDR_DEBUG
#define superfx_constraint_4addr_debug_fprintf(...) \
  do { fprintf (__VA_ARGS__); } while (0)
#define superfx_constraint_4addr_debug_rtx(...) \
  do { debug_rtx (__VA_ARGS__); } while (0)
#else
#define superfx_constraint_4addr_debug_fprintf(...) do { } while (0)
#define superfx_constraint_4addr_debug_rtx(...) do { } while (0)
#endif
...
#define SUPERFX_STACK_ALIGN(LOC) \
...
update_stack_args_size (struct machine_function* self)
{
  ...
  //superfx_debug_fprintf (stderr, "update_stack_args_size (): %i\n",
...
{
  // Padding needed for each element of the frame.
  self->local_vars_size = SUPERFX_STACK_ALIGN
  ...
  //superfx_debug_fprintf (stderr, "update_local_vars_size (): %i\n",
...
update_callee_saved_reg_size (struct machine_function* self)
{
  ...
  {
    if (superfx_regno_actually_callee_saved (regno))
    //if (superfx_regno_actually_callee_saved_no_hfp (regno))
    //if (df_regs_ever_live_p (regno) && !call_used_or_fixed_reg_p (regno))
    {
      self->callee_saved_reg_size += UNITS_PER_WORD;
    }
  }
  //superfx_debug_fprintf (stderr, "update_callee_saved_reg_size (): %i\n",
...
update_outgoing_args_size (struct machine_function* self)
{
  self->outgoing_args_size
    = ACCUMULATE_OUTGOING_ARGS
      ...
      : 0;
  //superfx_debug_fprintf (stderr, "update_outgoing_args_size (): %i\n",
  //  self->outgoing_args_size);
}
...
update_size_for_adjusting_sp (struct machine_function* self)
{
  ...
  self->size_for_adjusting_sp
    ...
    self->local_vars_size
    + self->outgoing_args_size;
  //superfx_debug_fprintf (stderr, "update_size_for_adjusting_sp (): %i\n",
  //  self->size_for_adjusting_sp);
}
static void
update_static_stack_size (struct machine_function* self)
{
  self->static_stack_size
    ...
    + self->local_vars_size
    + self->outgoing_args_size;
  //superfx_debug_fprintf (stderr, "update_static_stack_size (): %i\n",
  //  self->static_stack_size);
}

static void
superfx_compute_frame ()
{
...
do \
{ \
  superfx_operand_lossage (msgid, x); \
  return; \
} while (0)
...
//      GET_RTX_NAME (GET_CODE (x)),
//      GET_MODE_NAME (GET_MODE (x)));
//    superfx_dbg_print_rtx (SUBREG_REG (x));
//    //fprintf (stderr, ":%d",
//    //  SUBREG_BYTE (x));
//    fprintf (stderr, "]:");
//    print_poly_int (stderr, SUBREG_BYTE (x));
//  }
//  else if (superfx_absolute_ex (x))
//  {
//    fprintf (stderr, "%s:%s",
//      GET_RTX_NAME (GET_CODE (x)),
//      GET_MODE_NAME (GET_MODE (x)));
...
//      //GET_MODE_SIZE (GET_MODE (x))
//      GET_MODE_NAME (GET_MODE (x)));
//    superfx_dbg_print_rtx (XEXP (x, 0));
//    fprintf (stderr, "]");
//  }
...
//      //GET_MODE_SIZE (GET_MODE (x))
//      GET_MODE_NAME (GET_MODE (x)));
//    superfx_dbg_print_rtx (left);
//    fprintf (stderr, " ");
//    superfx_dbg_print_rtx (right);
//    fprintf (stderr, ")");
//  }
//  else
//  {
...
// We always return values in register r1 for superfx.
// This may need to be adjusted this for DImode?
static rtx
superfx_function_value (const_tree valtype,
  const_tree fntype_or_decl ATTRIBUTE_UNUSED,
  bool outgoing ATTRIBUTE_UNUSED)
{
  return gen_rtx_REG (TYPE_MODE (valtype), SUPERFX_R1);
}
...
// This may need to be adjusted this for DImode?
static rtx
superfx_libcall_value (machine_mode mode,
  const_rtx fun ATTRIBUTE_UNUSED)
{
  return gen_rtx_REG (mode, SUPERFX_R1);
}
...
// This may need to be adjusted this for DImode?
static bool
superfx_function_value_regno_p (const unsigned int regno)
{
  return (regno == SUPERFX_R1);
}

//static rtx
//superfx_static_chain (const_tree ARG_UNUSED (fndecl_or_type),
//  bool incoming_p)
//{
//  rtx addr, mem;
//
...
static void
superfx_asm_trampoline_template (FILE *f)
{
  ...
  ////// SuperFX that do not have any data cache.
...
//
//static void
//superfx_trampoline_init (rtx m_tramp, tree fndecl, rtx chain_value)
//{
...
static void
superfx_operand_lossage (const char *msgid, rtx op)
{
  debug_rtx (op);
...
static bool
superfx_valid_base_reg_p (rtx reg, bool strict_p)
{
  //const int regno = REGNO (reg);
  bool ret = false;
  superfx_debug_fprintf (stderr,
    "superfx_valid_base_reg_p () start: %d\n",
    int(strict_p));
  superfx_debug_rtx (reg);
  ...
  if (!strict_p && GET_CODE (reg) == SUBREG)
  {
    ...
    superfx_debug_fprintf (stderr, "!strict_p && SUBREG:\n");
    superfx_debug_rtx (reg);
  }
  ...
  )
  {
    if (strict_p)
    {
      superfx_debug_fprintf (stderr, "REG_P with strict_p: %d:%d:%d:%d\n",
        REGNO (reg),
    ...
    else
    {
      ...
      //  reg = SUBREG_REG (reg);
      //}
      superfx_debug_fprintf (stderr, "REG_P with !strict_p: %d:%d:%d\n",
        REGNO (reg),
  ...
  superfx_debug_fprintf (stderr, "superfx_valid_base_reg_p () end: %d\n",
    int(ret));
  superfx_debug_fprintf (stderr, "\n");
...
static bool
superfx_legitimate_address_p (machine_mode mode, rtx x, bool strict_p,
  addr_space_t as, code_helper my_code_helper ATTRIBUTE_UNUSED)
{
  superfx_debug_fprintf (stderr,
    "superfx_legitimate_address_p (): begin: %d\n",
    unsigned(GET_MODE_SIZE (mode)));

  gcc_assert (ADDR_SPACE_GENERIC_P (as));
  superfx_debug_fprintf (stderr,
    "superfx_legitimate_address_p (): post gcc_assert: %d\n",
    unsigned(GET_MODE_SIZE (mode)));
  ...
  // DImode
  {
    ...
    //{
    //  superfx_debug_fprintf (stderr,
    //    "superfx_legitimate_address_p (): dbg 0: %s\n",
    //    GET_MODE_NAME (mode));
    //  //superfx_debug_rtx (curr_insn);
    //  superfx_debug_rtx (x);
    //  superfx_debug_fprintf (stderr, "\n");
    //}
    if (GET_CODE (x) == REG)
    {
      if (superfx_valid_base_reg_p (x, strict_p))
      {
	return true;
      }
    }
    if (GET_CODE (x) == PLUS
      ...
      //&& superfx_valid_base_reg_p (XEXP (x, 0), strict_p)
    )
    {
      rtx
        left_0 = XEXP (x, 0),
        right_0 = XEXP (x, 1);
      superfx_debug_fprintf (stderr,
        "superfx_legitimate_address_p (): plus:\n");
      superfx_debug_rtx (left_0);
      superfx_debug_rtx (right_0);
      if (superfx_valid_base_reg_p (left_0, strict_p))
      {
	if (superfx_absolute_ex (right_0))
	{
	  return true;
	}
      }
  ...
  // --------
  if (superfx_absolute_ex (x))
  //if (superfx_absolute (x))
  {
    superfx_debug_fprintf (stderr,
      "superfx_legitimate_address_p (): absolute: %s\n",
      GET_MODE_NAME (mode));
    //superfx_debug_rtx (curr_insn);
    superfx_debug_rtx (x);
    return true;
  ...
  // --------
  superfx_debug_fprintf (stderr,
    "superfx_legitimate_address_p (): dbg: ");
  superfx_debug_rtx (x);
  return false;
}
...
static void
superfx_print_operand (FILE *file, rtx x, int code)
{
  rtx operand = x;
  ...
  switch (code)
  {
    ...
    default:
      //fprintf (stderr, "superfx_print_operand (): dbg 0\n");
  ...
  switch (GET_CODE (operand))
  {
    ...
    default:
      ...
      //fprintf (stderr, "superfx_print_operand (): dbg 1\n");
...
//static void
//superfx_print_addr_const (FILE* file, rtx x)
//{
//  switch (GET_CODE (x))
//  {
//    case LABEL_REF:
...
static void
superfx_print_operand_address (FILE* file,
  machine_mode mode ATTRIBUTE_UNUSED, rtx x)
{
  auto superfx_print_addr_const = [](FILE* file, rtx x) -> void {
    switch (GET_CODE (x))
    {
      case LABEL_REF:
	//output_addr_const (file, x);
  ...
  case PLUS:
	...
	// CONSTANT_P (right)
	superfx_absolute (right)
      )
      {
	//fprintf (file, "\"");
	//output_addr_const (file, right);
	//fprintf (file, "\"");
	superfx_print_addr_const (file, right);
      }
      else
      {
	gcc_unreachable ();
  ...
  //  break;
  case CONST:
    superfx_print_addr_const (file, x);
    break;
  case CONST_INT:
  //  //fprintf (file, "<some_symbol_ref>");
...
bool
superfx_offset_address_reg_plus_imm_p (rtx x)
{
  bool ret = false;
  superfx_constraint_debug_fprintf (stderr,
    "(offset base + imm) start:\n");
  superfx_constraint_debug_rtx (x);
  //superfx_constraint_debug_fprintf (stderr, "\n");
  ...
  if (MEM_P (x))
  {
	...
	= simplify_gen_subreg (SImode, x, DImode,
	  subreg_lowpart_offset (SImode, DImode));
      superfx_constraint_debug_fprintf (stderr, "x SUBREG:\n");
      superfx_constraint_debug_rtx (x);
    }
    if (GET_CODE (x) == REG)
    {
      ...
      if (GET_CODE (left_0) == SUBREG)
      {
        ...
        left_0
          = simplify_gen_subreg (SImode, left_0, DImode,
            subreg_lowpart_offset (SImode, DImode));
        superfx_constraint_debug_fprintf (stderr, "left_0 SUBREG:\n");
        superfx_constraint_debug_rtx (left_0);
      }
      //#endif // REG_OK_STRICT
      superfx_constraint_debug_fprintf (stderr,
        ...
        int(superfx_absolute_ex (right_0)));

      if (
        GET_CODE (left_0) == REG
        && superfx_absolute_ex (right_0)
      )
      {
        ret = true;
      }
...
int
superfx_initial_elimination_offset (int from, int to)
{
  int ret = 0;
  //--------
  // FL4SHK Note: seems that, at least for SuperFX, we want the offset as
  // (from) - (to).
...
// passed by reference.
static bool 
superfx_pass_by_reference (cumulative_args_t cum ATTRIBUTE_UNUSED,
  ...
  )
{
  ...
  return size > UNITS_PER_WORD * SUPERFX_NUM_ARG_REGS;
}
...
static int
superfx_arg_partial_bytes (cumulative_args_t cum_v ATTRIBUTE_UNUSED,
...
)
{
  ...
  if (*cum >= SUPERFX_NUM_ARG_REGS)
  {
    return 0;
  }

  if (superfx_pass_by_reference (cum_v, arg))
  {
    size = UNITS_PER_WORD;
  }
  else if (arg.type)
  ...
  bytes_left = (UNITS_PER_WORD * SUPERFX_NUM_ARG_REGS);
...
static rtx
superfx_function_arg (cumulative_args_t cum_v,
  ...
  )
{
  ...
  if ((*cum) <= SUPERFX_LAST_ARG_REGNUM)
  {
    // Check for trying to store larger-than-word-size arguments in
    // `cat(r5, r6)`
    if (GET_MODE_SIZE (mode) > UNITS_PER_WORD
      && (*cum) == SUPERFX_LAST_ARG_REGNUM)
    {
      return NULL_RTX;
    }

    const auto temp = (*cum);
    //fprintf (stderr, "\nsuperfx_function_arg REG:  %d\n", temp);
    return gen_rtx_REG (mode, temp);
  }
  else
  {
    //fprintf (stderr, "\nsuperfx_function_arg NULL_RTX\n");
    return NULL_RTX;
  }
}

#define SUPERFX_FUNCTION_ARG_SIZE(MODE, TYPE) \
...
// the stack.
static void
superfx_function_arg_advance (cumulative_args_t cum_v,
  //machine_mode mode,
  ...
  )
{
  ...
  *cum = (((*cum) <= SUPERFX_LAST_ARG_REGNUM)
    ? *cum + ((3 + SUPERFX_FUNCTION_ARG_SIZE (arg.mode, arg.type))
      / UNITS_PER_WORD)
    : *cum);
}
...
static void
superfx_option_override ()
{
  // Set the per-function-data initializer.
  init_machine_status = superfx_init_machine_status;
}

bool
superfx_regno_actually_callee_saved (int regno)
{
  ...
  {
    if (regno == SUPERFX_FAKE_FP || regno == SUPERFX_FAKE_AP)
    {
      return false;
    }

    if (!call_used_or_fixed_reg_p (regno) || regno == SUPERFX_LR)
    {
      return true;
    }
  }
...
static rtx
superfx_add3_frame_adjust (rtx op0, rtx op1, int addendum,
  const enum reg_note kind)
{
  ...
  //  && HARD_REGISTER_P (op0))
  //{
  //  fprintf (stderr, "superfx_add3_frame_adjust: %d\n",
  //    REGNO (op0));
  //}
  //if (GET_CODE (op1) == REG
  //  && HARD_REGISTER_P (op1))
...
static rtx
superfx_add_to_sp (int addendum, const enum reg_note kind)
{
  return superfx_add3_frame_adjust
    (stack_pointer_rtx, stack_pointer_rtx, addendum, kind);
}

static void
superfx_push (int regno, bool frame_related_p)
{
  ...
  //--------
  superfx_add_to_sp (-UNITS_PER_WORD, REG_NOTE_MAX);
  //--------
...
static void
superfx_partial_push (int regno, int idx, bool frame_related_p)
{
  rtx plus, mem, reg;
  rtx_insn* str_insn;
...
static void
superfx_partial_pop (int regno, int idx/*, bool frame_related_p*/)
{
  rtx plus, mem, reg;
  rtx_insn* ldr_insn;
  plus = gen_rtx_PLUS (SImode, stack_pointer_rtx, GEN_INT (idx * UNITS_PER_WORD));
...
static void//rtx
superfx_pop (int regno, int idx, bool finish=false)
{
  ...
  if (!finish)
  {
    superfx_partial_pop (regno, idx);
  }
  else
  {
    superfx_add_to_sp ((UNITS_PER_WORD * idx), REG_NOTE_MAX);
  }
...
superfx_expand_prologue ()
{
  ...
  {
    superfx_push (HARD_FRAME_POINTER_REGNUM, true);
    superfx_add3_frame_adjust (hard_frame_pointer_rtx, stack_pointer_rtx,
  ...
  for (int regno=0; regno<FIRST_PSEUDO_REGISTER; ++regno)
  {
    ...
    {
      //gen_push (regno);
      superfx_partial_push (regno, idx, true);
      ++idx;
    }
  }
...
superfx_expand_epilogue ()
{
  ...
  if (cfun->machine->size_for_adjusting_sp > 0)
  {
    superfx_add_to_sp (cfun->machine->size_for_adjusting_sp,
      REG_FRAME_RELATED_EXPR);
  }
  ...
  for (int regno=FIRST_PSEUDO_REGISTER-1; regno>=0; --regno)
  {
    ...
    {
      //gen_pop (regno);
      superfx_pop (regno, idx);
      ++idx;
    }
  }

  // Restore the frame pointer.
  //if (df_regs_ever_live_p (SUPERFX_FP))
  ...
  if (frame_pointer_needed)
  {
    ...
    superfx_pop (HARD_FRAME_POINTER_REGNUM, idx);
    ++idx;
  }
  superfx_pop (0, idx, true);
...
void
superfx_emit_mov (rtx dst, rtx src, machine_mode mode)
{
  ...
  superfx_debug_fprintf (stderr,
    ...
    reload_completed,
    GET_MODE_NAME (mode));
  superfx_debug_rtx (operands[0]);
  superfx_debug_rtx (operands[1]);
  ...
  if (!(reload_in_progress || reload_completed))
  {
    if (
      //can_create_pseudo_p () && 
      MEM_P (operands[0])
    //&& !superfx_mov_reg_operand (mode, operands[1])
    )
    {
      superfx_debug_fprintf (stderr, "MEM_P (operands[0])\n");
      {
      ...
      if (MEM_P (XEXP (operands[0], 0))
        || superfx_absolute_ex (XEXP (operands[0], 0)))
      {
        rtx tmp;
        superfx_debug_fprintf (stderr,
          "operands[0] = gen_rtx_MEM (...):\n");
        superfx_debug_rtx (XEXP (operands[0], 0));
        tmp = force_reg (Pmode, XEXP (operands[0], 0));
        superfx_debug_rtx (tmp);
      ...
      {
        superfx_debug_fprintf (stderr,
          "don't have inner `MEM_P`/`superfx_absolute_ex`\n");
      }
    }
    else if (
      //can_create_pseudo_p () &&
    ...
    {
      superfx_debug_fprintf (stderr, "MEM_P (operands[1])\n");
      if (MEM_P (XEXP (operands[1], 0))
        || superfx_absolute_ex (XEXP (operands[1], 0)))
      {
        rtx tmp;
        superfx_debug_fprintf (stderr,
          "operands[1] = gen_rtx_MEM (...):\n");
        superfx_debug_rtx (XEXP (operands[1], 0));
...
rtx
superfx_get_return_address (int count)
{
  if (count != 0)
  {
    return NULL_RTX;
  ...
  return get_hard_reg_initial_val (Pmode, SUPERFX_LR);
}
static void
superfx_asm_named_section (const char *name, unsigned int flags, tree decl)
{
  default_elf_asm_named_section (name, flags, decl);
}

static bool
superfx_hard_regno_mode_ok (unsigned int regno, machine_mode mode)
{
  if (GET_MODE_CLASS (mode) == MODE_CC)
  {
    //return (regno == SUPERFX_FLAGS);
    return (regno <= SUPERFX_LAST_GENERAL_REGNUM);
  }
  //if (regno == SUPERFX_FLAGS && GET_MODE_CLASS (mode) != MODE_CC)
  //{
  //  return false;
  //}
  ...
  int *pretend_args_size, int no_rtl)
{
  ...
  *pretend_args_size = (nregs < 0) ? 0 : (GET_MODE_SIZE (SImode) * nregs);
  ...
  for (int regno=*cum; regno<SUPERFX_NUM_ARG_REGS; ++regno)
  {
...
//static void
//superfx_emit_compare (enum rtx_code *code, rtx *op0, rtx *op1)
//{
//  rtx cmp_op0 = *op0;
//  rtx cmp_op1 = *op1;
...
//const char*
//superfx_expand_conditional_branch (rtx *operands)
//{
...
//--------
//void
//superfx_gen_cmpdi_reg (rtx x, rtx y)
//{
//  //machine_mode mode;
//  //rtx cc_reg;
//
...
//  emit_insn (gen_superfx_cmpsi3_carry_in (scratch, x_hi, y_hi));
//}
//--------
...
#define TARGET_RETURN_IN_MEMORY superfx_return_in_memory
#undef TARGET_MUST_PASS_IN_STACK
#define TARGET_MUST_PASS_IN_STACK must_pass_in_stack_var_size
//#undef TARGET_MUST_PASS_IN_STACK
//#define TARGET_MUST_PASS_IN_STACK superfx_must_pass_in_stack
//#define TARGET_MUST_PASS_IN_STACK must_pass_in_stack_var_size
#undef  TARGET_PASS_BY_REFERENCE
#define TARGET_PASS_BY_REFERENCE superfx_pass_by_reference
#undef  TARGET_ARG_PARTIAL_BYTES
#define TARGET_ARG_PARTIAL_BYTES superfx_arg_partial_bytes
//#define TARGET_ARG_PARTIAL_BYTES hook_bool_void_false
#undef TARGET_FUNCTION_ARG
#define TARGET_FUNCTION_ARG superfx_function_arg
#undef  TARGET_FUNCTION_ARG_ADVANCE
#define TARGET_FUNCTION_ARG_ADVANCE superfx_function_arg_advance
...
#define TARGET_ADDR_SPACE_LEGITIMATE_ADDRESS_P superfx_legitimate_address_p

#undef  TARGET_SETUP_INCOMING_VARARGS
#define TARGET_SETUP_INCOMING_VARARGS superfx_setup_incoming_varargs

//#undef TARGET_SECONDARY_RELOAD
//#define TARGET_SECONDARY_RELOAD superfx_secondary_reload

//#undef TARGET_FIXED_CONDITION_CODE_REGS
//#define TARGET_FIXED_CONDITION_CODE_REGS superfx_fixed_condition_code_regs

//#define TARGET_SPILL_CLASS superfx_target_spill_class

// Define this to return an RTX representing the place where a
// function returns or receives a value of data type RET_TYPE, a tree
// node representing a data type.
#undef TARGET_FUNCTION_VALUE
#define TARGET_FUNCTION_VALUE superfx_function_value
#undef TARGET_LIBCALL_VALUE
#define TARGET_LIBCALL_VALUE superfx_libcall_value
#undef TARGET_FUNCTION_VALUE_REGNO_P
#define TARGET_FUNCTION_VALUE_REGNO_P superfx_function_value_regno_p
...
//#define TARGET_STATIC_CHAIN superfx_static_chain

//#undef TARGET_ASM_TRAMPOLINE_TEMPLATE
//#define TARGET_ASM_TRAMPOLINE_TEMPLATE superfx_asm_trampoline_template
//#undef TARGET_TRAMPOLINE_INIT
//#define TARGET_TRAMPOLINE_INIT superfx_trampoline_init

#undef TARGET_HARD_REGNO_MODE_OK 
#define TARGET_HARD_REGNO_MODE_OK superfx_hard_regno_mode_ok

#undef TARGET_ASM_NAMED_SECTION 
#define TARGET_ASM_NAMED_SECTION superfx_asm_named_section

//#undef TARGET_RTX_COSTS
//#define TARGET_RTX_COSTS superfx_target_rtx_costs
//
//#undef TARGET_MEMORY_MOVE_COST
//#define TARGET_MEMORY_MOVE_COST superfx_target_memory_move_cost

#undef TARGET_OPTION_OVERRIDE
#define TARGET_OPTION_OVERRIDE superfx_option_override

#undef  TARGET_PRINT_OPERAND
#define TARGET_PRINT_OPERAND superfx_print_operand
#undef  TARGET_PRINT_OPERAND_ADDRESS
#define TARGET_PRINT_OPERAND_ADDRESS superfx_print_operand_address

#undef  TARGET_CONSTANT_ALIGNMENT
#define TARGET_CONSTANT_ALIGNMENT constant_alignment_word_strings

//#undef TARGET_PREFERRED_RELOAD_CLASS
//#define TARGET_PREFERRED_RELOAD_CLASS superfx_preferred_reload_class

struct gcc_target targetm = TARGET_INITIALIZER;

#include "gt-superfx.h"
